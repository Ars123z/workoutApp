...
fun GlucoseScreen(
    ...
    glucoseViewModel: GlucoseViewModel = viewModel(
        factory = GlucoseViewModel.factory
    ),
    onResumeAvailabilityCheck: () -> Unit,
    lifecycleOwner: LifecycleOwner = LocalLifecycleOwner.current
) {
    val permissionsGranted by glucoseViewModel.permissionsGranted
    val sessionsList by glucoseViewModel.sessionsList
    val permissions = glucoseViewModel.permissions
    val onPermissionsResult = { glucoseViewModel.initialLoad() }
    val permissionsLauncher =
        rememberLauncherForActivityResult(glucoseViewModel.permissionsLauncher) {
            onPermissionsResult()
        }
    ...
    Column(
        ...
        when (healthConnectAvailability) {
            HealthConnectAvailability.INSTALLED -> Glucose(
                permissions = permissions,
                permissionsGranted = permissionsGranted,
                sessionsList = sessionsList,
                uiState = glucoseViewModel.uiState,
                onPermissionsResult = { glucoseViewModel.initialLoad() },
                onPermissionsLaunch = { values ->
                    permissionsLauncher.launch(values)
                },
                onInsertClick = { glucoseViewModel.insertBloodGlucoseRecord() },
            )
...
fun Glucose(
    ...
    sessionsList: List<BloodGlucoseRecord>,
    uiState: GlucoseViewModel.UIState,
    onInsertClick: () -> Unit = {},
    ...
    LaunchedEffect(uiState) {
        ...
        if (uiState is GlucoseViewModel.UIState.Uninitialized) {
            onPermissionsResult()
        }
        ...
        if (uiState is GlucoseViewModel.UIState.Error && errorId.value != uiState.uuid) {
