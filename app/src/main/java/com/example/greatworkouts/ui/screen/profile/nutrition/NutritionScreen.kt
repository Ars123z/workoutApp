package com.example.greatworkouts.ui.screen.profile.nutrition
...
fun NutritionScreen(
    ...
    val permissionsGranted by nutritionViewModel.permissionsGranted
    val sessionsList by nutritionViewModel.sessionsList
    val permissions = nutritionViewModel.permissions
    val onPermissionsResult = { nutritionViewModel.initialLoad() }
    val permissionsLauncher =
        rememberLauncherForActivityResult(nutritionViewModel.permissionsLauncher) {
            onPermissionsResult()
        }
    ...
    Column(
        ...
        when (healthConnectAvailability) {
            HealthConnectAvailability.INSTALLED -> Nutrition(
                permissions = permissions,
                permissionsGranted = permissionsGranted,
                sessionsList = sessionsList,
                uiState = nutritionViewModel.uiState,
                onPermissionsResult = { nutritionViewModel.initialLoad() },
                onPermissionsLaunch = { values ->
                    permissionsLauncher.launch(values)
                },
                onInsertClick = { nutritionViewModel.insertTotalCaloriesBurnedRecord() },
            )
...
fun Nutrition(
    ...
    sessionsList: List<NutritionRecord>,
    uiState: NutritionViewModel.UIState,
    onInsertClick: () -> Unit = {},
    ...
    LaunchedEffect(uiState) {
        ...
        if (uiState is NutritionViewModel.UIState.Uninitialized) {
            onPermissionsResult()
        }
        ...
        if (uiState is NutritionViewModel.UIState.Error && errorId.value != uiState.uuid) {
        ...
        LazyColumn(
            ...
            if (!permissionsGranted) {
                ...
                items(sessionsList) { session ->
                    Text(
                        text= "Calcium: ${session.calcium.toString()}"
                    )
                    Text(
                        text= "Energy: ${session.energy.toString()}"
                    )
                    Text(
                        text= "Carbohydrates: ${session.cholesterol.toString()}"
                    )
                }
            }
        }
