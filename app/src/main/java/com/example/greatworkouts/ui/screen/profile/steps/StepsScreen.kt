...
fun StepsScreen(
    ...
    stepsViewModel: StepsViewModel = viewModel(
        factory = StepsViewModel.factory
    ),
    onResumeAvailabilityCheck: () -> Unit,
    lifecycleOwner: LifecycleOwner = LocalLifecycleOwner.current
) {
    val permissionsGranted by stepsViewModel.permissionsGranted
    val sessionsList by stepsViewModel.sessionsList
    val permissions = stepsViewModel.permissions
    val onPermissionsResult = { stepsViewModel.initialLoad() }
    val permissionsLauncher =
        rememberLauncherForActivityResult(stepsViewModel.permissionsLauncher) {
            onPermissionsResult()
        }
    ...
    Column(
        ...
        when (healthConnectAvailability) {
            HealthConnectAvailability.INSTALLED -> Sleep(
                ...
                onPermissionsResult = { stepsViewModel.initialLoad() },
                onPermissionsLaunch = { values ->
                    permissionsLauncher.launch(values)
                },
                onInsertClick = { stepsViewModel.insertStepsRecord() },
            )
