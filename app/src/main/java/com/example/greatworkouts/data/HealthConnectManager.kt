...
import com.example.greatworkouts.workers.ReadStepWorker
...
class HealthConnectManager(private val context: Context) {
    ...
    suspend fun writeExerciseSession(start: ZonedDateTime, end: ZonedDateTime) {
        Log.d("HealthConnect", "Writing exercise session")
    ...
    /**
     * Write Blood Glucose Records
     */

    suspend fun writeBloodGlucoseRecord(start: ZonedDateTime){

        val bloodGlucoseRecord = BloodGlucoseRecord(
            time = start.toInstant(),
            zoneOffset = start.offset,
            level = BloodGlucose.milligramsPerDeciliter(100.0 + Random.nextDouble())
        )
        healthConnectClient.insertRecords(listOf(bloodGlucoseRecord))
    }
    ...
    /**
     * Write Total Calories Burned Records
     */

    suspend fun writeNutritionRecord(start: ZonedDateTime, end: ZonedDateTime) {
        val nutritionRecord = NutritionRecord(
            startTime = start.toInstant(),
            endTime = end.toInstant(),
            startZoneOffset = start.offset,
            endZoneOffset = end.offset,
            calcium = Mass.milligrams(20.0),
            cholesterol = Mass.milligrams(10.0),
            energy = Energy.kilocalories(100.0),
            biotin = Mass.milligrams(20.0)
        )
    ...
    /**
     * Read Total Calories Burned Records
     */
    ...
    suspend fun writeStepsRecord(start: ZonedDateTime, end: ZonedDateTime) {
        healthConnectClient.insertRecords(
            listOf(
                StepsRecord(
            startTime = start.toInstant(),
